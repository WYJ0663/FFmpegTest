...
const char *infile = "/sdcard/22test_audio.wav";
const char *outfile = "/sdcard/22demo.aac";

void usage(const char *name) {
    LOGE("%s [-r bitrate] [-t aot] [-a afterburner] [-s sbr] [-v vbr] in.wav out.aac\n", name);
    LOGE("Supported AOTs:\n");
    LOGE("\t2\tAAC-LC\n");
    LOGE("\t5\tHE-AAC\n");
    LOGE("\t29\tHE-AAC v2\n");
...
//	outfile = argv[optind + 1];
    ...
    if (!wav) {
        LOGE("Unable to open wav file %s\n", infile);
        return 1;
    }
    ...
    LOGE("sample_rate %d ,bits_per_sample %d ,channels %d ,format  %d ", sample_rate, bits_per_sample, channels, format)
    if (format != 1) {
        LOGE("Unsupported WAV format %d\n", format);
        return 1;
    }
    if (bits_per_sample != 16) {
        LOGE("Unsupported WAV sample depth %d\n", bits_per_sample);
        return 1;
    }
